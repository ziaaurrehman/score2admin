  ...
  return (
    ...
    >
      {loading ? (
        ...
        >
          {blocked.map((block, index) => (
            <div
              key={index}
              className="w-max bg-slate-300 rounded-md text-xs flex"
            >
              <p className="border-r border-slate-500 p-1">{block}</p>
              <p
                className="transition-color cursor-pointer hover:text-red-500 p-1"
                onClick={() => handleRemoveCountry(block)}
              >
                x
              </p>
            </div>
      ...
      <div className="w-full flex flex-col gap-2">
        <label className="font-bold">Block Countries</label>
        <select
          className="border-2 border-gray-300 rounded-md py-[0.2rem] cursor-pointer w-[100%]"
          name="block_countries"
          onChange={(e) => handleBlockCountry(e.target.value)}
        >
